...
public:
  ...
  // Helper functions.
    ...
    std::string getArgument(const char *Name, bool Value);

    /// \brief Append an int argument to the top trace item.
    std::string getArgument(const char *Name, int Value);

    /// \brief Append a string argument to the top trace item.
    std::string getArgument(const char *Name, const char *Value);

    /// \brief Append a string reference object argument to the top trace item.
    std::string getArgument(const char *Name, llvm::StringRef Value);

    /// \brief Append a token argument to the top trace item.
    std::string getArgument(const char *Name, const clang::Token &Value);

    /// \brief Append an enum argument to the top trace item.
    std::string getArgument(const char *Name, int Value, const char *const Strings[]);
    ...
    std::string getArgument(const char *Name, clang::SourceRange Value);

    /// \brief Append a CharSourceRange argument to the top trace item.
    std::string getArgument(const char *Name, clang::CharSourceRange Value);

    /// \brief Append a ModuleIdPath argument to the top trace item.
    std::string getArgument(const char *Name, clang::ModuleIdPath Value);

    /// \brief Append an IdentifierInfo argument to the top trace item.
    std::string getArgument(const char *Name, const clang::IdentifierInfo *Value);

    /// \brief Append a MacroDirective argument to the top trace item.
    std::string getArgument(const char *Name, const clang::MacroDirective *Value);

    /// \brief Append a MacroDefinition argument to the top trace item.
    std::string getArgument(const char *Name, const clang::MacroDefinition &Value);

    /// \brief Append a MacroArgs argument to the top trace item.
    std::string getArgument(const char *Name, const clang::MacroArgs *Value);

    /// \brief Append a Module argument to the top trace item.
    std::string getArgument(const char *Name, const clang::Module *Value);

    /// \brief Append a double-quoted argument to the top trace item.
    std::string getQuotedArgument(const char *Name, const std::string &Value);

    /// \brief Append a double-quoted file path argument to the top trace item.
    std::string getFilePathArgument(const char *Name, llvm::StringRef Value);
