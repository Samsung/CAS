...
This is the clang processor used to parse source files (currently only C sources are supported) to extract relevant information from the parsed representation and store it in simple JSON database file. More details can be found in [readme](clang-proc/README.md) file.
