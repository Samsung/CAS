...
#define SHOW_HASH(v) printf("#define PHASH_%s %d\n",v,simple_hash(v))

void COMPUTE_SHOW_HASHES() {
	SHOW_HASH("New_proc");
	SHOW_HASH("SchedFork");
	SHOW_HASH("SysClone");
	SHOW_HASH("Close");
	SHOW_HASH("Pipe");
	SHOW_HASH("Open");
	SHOW_HASH("RenameFrom");
	SHOW_HASH("Rename2From");
	SHOW_HASH("LinkFrom");
	SHOW_HASH("LinkatFrom");
	SHOW_HASH("Symlink");
	SHOW_HASH("RenameTo");
	SHOW_HASH("RenameFailed");
	SHOW_HASH("LinkTo");
	SHOW_HASH("LinkFailed");
	SHOW_HASH("Exit");
	SHOW_HASH("pid");
	SHOW_HASH("time");
	SHOW_HASH("timen");
	...
	SHOW_HASH("fnamesize");
	SHOW_HASH("forigsize");
	SHOW_HASH("targetnamesize");
	SHOW_HASH("linknamesize");
	SHOW_HASH("resolvednamesize");
	SHOW_HASH("Progname_i");
	SHOW_HASH("Progname_p");
	SHOW_HASH("End_of_args");
	SHOW_HASH("fd1");
	SHOW_HASH("fd2");
	SHOW_HASH("cpu");
	SHOW_HASH("cwd");
	SHOW_HASH("prognameisize");
	SHOW_HASH("prognamepsize");
	SHOW_HASH("cwdsize");
	SHOW_HASH("Cont");
	SHOW_HASH("Cont_end");
	SHOW_HASH("SysCloneFailed");
	SHOW_HASH("Dup");
	SHOW_HASH("oldfd");
	SHOW_HASH("newfd");
}
...
static int process_arg(char* arg, enum SYSEVENT cmd, void* cmdArgs) {
	...
	*eargname = 0;
	...
	switch(simple_hash(arg)) {
		...
		{
			assert(cmd==SYSEVENT_NEWPROC);
			if (sscanf(eargname+1,SYSEVENT_NEWPROC_ARG_PROGNAMEISIZE_FMT,&((struct NewProcArgs*)cmdArgs)->prognameisize)==1) {
				goto done;
			}
		}
		break;
		...
		{
			assert(cmd==SYSEVENT_NEWPROC);
			if (sscanf(eargname+1,SYSEVENT_NEWPROC_ARG_PROGNAMEPSIZE_FMT,&((struct NewProcArgs*)cmdArgs)->prognamepsize)==1) {
				goto done;
			}
		}
		break;
		...
		{
			assert((cmd==SYSEVENT_SYSCLONE)||(cmd==SYSEVENT_OPEN)||(cmd==SYSEVENT_RENAME2FROM)||(cmd==SYSEVENT_LINKATFROM)
					||(cmd==SYSEVENT_PIPE)||(cmd==SYSEVENT_DUP));
		...
		{
			assert((cmd==SYSEVENT_OPEN)||(cmd==SYSEVENT_RENAMEFROM)||(cmd==SYSEVENT_RENAME2FROM)||(cmd==SYSEVENT_RENAMETO)||
					(cmd==SYSEVENT_LINKFROM)||(cmd==SYSEVENT_LINKATFROM)||(cmd==SYSEVENT_LINKTO));
		...
		case PHASH_STR(targetnamesize):
		{
			...
			if (sscanf(eargname+1,SYSEVENT_SYMLINK_ARG_TARGETNAMESIZE_FMT,&((struct SymLinkArgs*)cmdArgs)->targetnamesize)==1) {
				goto done;
			}
		}
		break;
		...
		{
			assert(cmd==SYSEVENT_SYMLINK);
			if (sscanf(eargname+1,SYSEVENT_SYMLINK_ARG_LINKNAMESIZE_FMT,&((struct SymLinkArgs*)cmdArgs)->linknamesize)==1) {
				goto done;
			}
		}
		break;
		...
		{
			assert(cmd==SYSEVENT_SYMLINK);
			if (sscanf(eargname+1,SYSEVENT_SYMLINK_ARG_RESOLVEDNAMESIZE_FMT,&((struct SymLinkArgs*)cmdArgs)->resolvednamesize)==1) {
				goto done;
			}
		}
		break;
...
int parse_generic_args(const char* line, eventTuple_t* evln) {
	...
	const char* pide = strchr(line,',');
	if (pide) {
		if (sscanf(line,GENERIC_ARG_PID_FMT,&evln->pid)!=1) {
			return 0;
		}
		const char* cpue = strchr(pide+1,',');
		if (cpue) {
			if (sscanf(pide+1,GENERIC_ARG_CPU_FMT,&evln->cpu)!=1) {
				return 0;
			}
			const char* timee = strchr(cpue+1,',');
			if (timee) {
				if (sscanf(cpue+1,GENERIC_ARG_TIME_FMT,&evln->time)!=1) {
					return 0;
				}
				const char* timene = strchr(timee+1,'!');
				if (timene) {
					if (sscanf(timee+1,GENERIC_ARG_TIMEN_FMT,&evln->timen)!=1) {
						return 0;
					}
					const char* cmde = strchr(timene+1,'|');
					if (cmde) {
						evln->event = simple_hashn(timene+1,cmde-timene+1);
					}
					else {
						const char* cmd = timene;
						if ((*(++cmd)=='A') && (*(++cmd)=='[')) {
							while (isdigit(*(++cmd)));
							if (*cmd==']') goto done_ok;
						}
...
done_ok:
					evln->event_line = strdup(timene+1);
					return 1;
				}
			}
		}
	...
	do {
		...
		if (end) {
			...
			*end=',';
			...
			if (!process_arg(beg,cmd,cmdArgs)) return 0;
		}
	}
	while(end);
	return 1;
...
static char* get_longpath_arg(vpeventTuple_t* event_list, unsigned long* vi, const char* tag, const char* tag_end) {
	...
	while(*vi<VEC_SIZE(*event_list)) {
		...
		else if ((ievln->event_line[0]==tag[0])&&(ievln->event_line[1]==tag[1])&&(ievln->event_line[2]=='[')) {
	...
	while(vi<VEC_SIZE(*event_list)) {
		...
		switch (event) {
				...
				while(vi<VEC_SIZE(*event_list)) {
					...
					if ((ievln->event_line[0]=='P')&&(ievln->event_line[1]=='I')) {
						...
						if (strlen(longfn)!=argnfo.prognameisize) {
							fprintf(stderr,"ERROR: Size mismatch in progname_i (%zu vs %zu) exec argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
						...
						if (strlen(longfn)!=argnfo.prognamepsize) {
							fprintf(stderr,"ERROR: Size mismatch in progname_p (%zu vs %zu) exec argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
						...
						if (strlen(longfn)!=argnfo.cwdsize) {
							fprintf(stderr,"ERROR: Size mismatch in cwd (%zu vs %zu) exec argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
						...
						*cmde = ']';
						...
						if (simple_hashn(ievln->event_line,cmde-ievln->event_line)==PHASH_STR(End_of_args)) {
							...
							VEC_APPEND(const char*,argnfo.argv,longargs);
							unsigned long ai;
							uint64_t utn = (uint64_t)evln->time*(uint64_t)1000000000UL+evln->timen;
							...
							exe_start_time = utn;
							if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
							if (context->rawoutfd)
								fprintf(context->rawoutfd,"{\"c\":\"e\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"b\":\"%s\",\"w\":\"%s\",\"v\":[",
							...
							for(ai=0;ai<VEC_SIZE(argnfo.argv); ++ai) {
								...
								if (context->rawoutfd) fprintf(context->rawoutfd,"\"%s\"",argi);
								FREE_JSON_ESCAPED(argi);
								if ((ai<VEC_SIZE(argnfo.argv)-1)&&(context->rawoutfd)) fprintf(context->rawoutfd,",");
							}
			...
			case PHASH_STR(SchedFork):
			{
				...
				if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
				if (context->rawoutfd)
					fprintf(context->rawoutfd,"{\"c\":\"f\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"h\":" GENERIC_ARG_PID_FMT ",\"f\":%lu}",
				...
				context->rev_fork_map.insert(std::pair<upid_t,std::pair<upid_t,unsigned>>(argnfo.cpid,
						std::pair<upid_t,unsigned>(evln->pid,context->ve.size()-ve_init_size-1)));
				n_processed++;
			...
			case PHASH_STR(SysClone):
			{
				...
				if (vi>=VEC_SIZE(*event_list)) {
					fprintf(stderr,"WARNING: Missing SchedFork/SysCloneFailed after Clone at [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",vi,evln->pid,evln->event_line);
					break;
				}
				eventTuple_t* ievln = VEC_ACCESS(*event_list,vi);
				char* cmde = strchr(ievln->event_line,'|');
				...
				if (event==PHASH_STR(SysCloneFailed)) {
					...
					context->rev_fork_map.insert(std::pair<upid_t,std::pair<upid_t,unsigned>>(argnfo_fork.cpid,
							std::pair<upid_t,unsigned>(evln->pid,context->ve.size()-ve_init_size-1)));
					n_processed+=2;
					context->event_count.fork++;
					srvec.push_back(syscall_raw(pid,syscall_raw::SYS_FORK,argnfo_fork.cpid,argnfo.flags,utn-context->start_time));
				}
			}
			break;
			case PHASH_STR(Close):
			{
				...
				if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
				if (context->rawoutfd)
					fprintf(context->rawoutfd,"{\"c\":\"c\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"fd\":%d}",
						evln->pid,procidx,utn-context->start_time,argnfo.fd);
				n_processed++;
			...
			case PHASH_STR(Pipe):
			{
				...
				if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
				if (context->rawoutfd)
					fprintf(context->rawoutfd,"{\"c\":\"p\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"fdr\":%d,\"fdw\":%d,\"f\":%lu}",
						evln->pid,procidx,utn-context->start_time,argnfo.fd1,argnfo.fd2,argnfo.flags);
				n_processed++;
				context->event_count.pipe++;
				srvec.push_back(syscall_raw(pid,syscall_raw::SYS_PIPE,
						argnfo.fd1,argnfo.fd2,argnfo.flags,utn-context->start_time));
			}
			break;
			case PHASH_STR(Dup):
			{
				...
				if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
				if (context->rawoutfd)
					fprintf(context->rawoutfd,"{\"c\":\"d\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"ofd\":%d,\"nfd\":%d,\"f\":%lu}",
						evln->pid,procidx,utn-context->start_time,argnfo.oldfd,argnfo.newfd,argnfo.flags);
				n_processed++;
				context->event_count.dup++;
				srvec.push_back(syscall_raw(pid,syscall_raw::SYS_DUP,
						argnfo.oldfd,argnfo.newfd,argnfo.flags,utn-context->start_time));
			}
			break;
			case PHASH_STR(Open):
			{
				...
				if (!longfn) {
					fprintf(stderr,"WARNING: Invalid open argument line [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",vi,evln->pid,
							VEC_ACCESS(*event_list,vi)->event_line);
					continue;
				}
				if (strlen(longfn)!=argnfo.fnamesize) {
					fprintf(stderr,"ERROR: Size mismatch in fname (%zu vs %zu) open argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
				...
				if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
				if (context->rawoutfd)
					fprintf(context->rawoutfd,"{\"c\":\"o\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"h\":\"%s\",\"o\":\"%s\",\"f\":%lu,\"m\":%ld,\"fd\":%d}",
						evln->pid,procidx,utn-context->start_time,_longfn,_longfo,argnfo.flags,argnfo.mode,argnfo.fd);
			...
			case PHASH_STR(Rename2From):
			{
				...
				if (strlen(longfn_RF)!=argnfo_RF.fnamesize) {
					fprintf(stderr,"ERROR: Size mismatch in fname (%zu vs %zu) rename argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
							strlen(longfn_RF),argnfo_RF.fnamesize,vi,evln->pid,evln->event_line);
					goto error;
				}
				/* Next event should be RenameTo or RenameFailed */
				++vi;
				if (vi>=VEC_SIZE(*event_list)) break;
				eventTuple_t* ievln = VEC_ACCESS(*event_list,vi);
				char* cmde = strchr(ievln->event_line,'|');
				if ((simple_hashn(ievln->event_line,cmde-ievln->event_line))==PHASH_STR(RenameTo)) {
					...
					if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
					if (context->rawoutfd)
						fprintf(context->rawoutfd,"{\"c\":\"r\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"o\":\"%s\",\"n\":\"%s\",\"f\":%lu}",
					...
					fprintf(stderr,"WARNING: Invalid event after Rename(2)From (" GENERIC_ARG_PID_FMT ")[%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",pid,vi,ievln->pid,ievln->event_line);
					free((void*)longfn_RF);
					vi--;
					break;
				}
			...
			case PHASH_STR(LinkatFrom):
			{
				...
				if (strlen(longfn_LF)!=argnfo_LF.fnamesize) {
					fprintf(stderr,"ERROR: Size mismatch in fname (%zu vs %zu) link argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
				...
				if ((simple_hashn(ievln->event_line,cmde-ievln->event_line))==PHASH_STR(LinkTo)) {
					...
					if ((context->total_event_count+n_processed>0)&&(context->rawoutfd)) fprintf(context->rawoutfd,",\n");
					if (context->rawoutfd)
						fprintf(context->rawoutfd,"{\"c\":\"l\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"o\":\"%s\",\"n\":\"%s\",\"f\":%lu}",
					...
					fprintf(stderr,"WARNING: Invalid event after Link(At)From (" GENERIC_ARG_PID_FMT ")[%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",pid,vi,ievln->pid,ievln->event_line);
					free((void*)longfn_LF);
					vi--;
					break;
				}
			...
			case PHASH_STR(Symlink):
			{
				...
				if (strlen(longfn_ST)!=argnfo.targetnamesize) {
					fprintf(stderr,"ERROR: Size mismatch in targetnamesize (%zu vs %zu) symlink argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
				...
				if (longfn_SR) {
					if (strlen(longfn_ST)!=argnfo.targetnamesize) {
						fprintf(stderr,"ERROR: Size mismatch in resolvednamesize (%zu vs %zu) symlink argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
				...
				if (strlen(longfn_SL)!=argnfo.linknamesize) {
					fprintf(stderr,"ERROR: Size mismatch in linknamesize (%zu vs %zu) symlink argument [%lu]: [" GENERIC_ARG_PID_FMT "] %s\n",
				...
				if (context->rawoutfd) {
					if (_longfn_SR) {
						fprintf(context->rawoutfd,"{\"c\":\"s\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64
								",\"o\":\"%s\",\"r\":\"%s\",\"n\":\"%s\"}",
								evln->pid,procidx,utn-context->start_time,_longfn_ST,_longfn_SR,_longfn_SL);
					}
					else {
						fprintf(context->rawoutfd,"{\"c\":\"s\",\"p\":" GENERIC_ARG_PID_FMT ",\"x\":%u,\"t\":%" PRIu64 ",\"o\":\"%s\",\"n\":\"%s\"}",
			...
			case PHASH_STR(Exit):
			{
				...
				uint64_t utn = (uint64_t)evln->time*(uint64_t)1000000000UL+evln->timen;
				if (context->start_time==0) context->start_time=utn;
				n_processed++;
...
error:
        ...
        context->total_event_count+=n_processed;
        context->process_count++;
        for (size_t u=0; u<srvec.size()-srvec_init_size; ++u) srvec.pop_back();
        context->event_count.exit++;
