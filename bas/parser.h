...
struct parsed_entry {
    ...
    void addUpdateFile(const char *p, unsigned flags, upid_t pid = -1, const char *o = 0) {
        ...
        if (this->rwmap.find(k) != this->rwmap.end()) {
            ...
            this->rwmap.insert(std::pair<std::pair<std::string, std::string>, std::pair<unsigned, long>>(
                k, std::pair<unsigned, unsigned>(flags & 0x3, 0)));
        }
        if (access(k.first.c_str(), F_OK) != -1) {
            ...
            unsigned mode = 0x40 | ((path_stat.st_mode & S_IFMT) >> 10);
