...
async function loadChildren(node: ProcessWithChildren) {
	const eid =
		node.pid === proc?.pid && node.idx === proc?.idx
			? { pid: node.ppid, idx: node.pidx }
			: { pid: node.pid, idx: node.idx };
	if (!eid.pid && !eid.idx) {
		console.error("no eid");
		throw new Error("no eid");
	}
	// const currentHeight = node.height;
	const { data: newProc } = (await vscode.execFunction("loadChildren", {
		data: eid,
	})) as { data: ProcessWithChildren };
	if (Array.isArray(newProc.children)) {
		newProc.children = filterEmpty(newProc.children);
	}
	return newProc;
}
async function setRoot(node: ProcessWithChildren) {
	const eid =
		node.pid === proc?.pid && node.idx === proc?.idx
			? { pid: node.ppid, idx: node.pidx }
			: { pid: node.pid, idx: node.idx };
	...
	// const currentHeight = node.height;
	({ data: proc } = (await vscode.execFunction("loadChildren", {
		data: eid,
	})) as { data: ProcessWithChildren });
	if (Array.isArray(proc.children)) {
		proc.children = filterEmpty(proc.children);
