...
function nextFiles(maxResults=50) {
    ...
    if (page < (parseInt(files_pages.innerText.split("/")[1]) - 1)) {
        page++;
        let request = '/deps_for?path=' + deps_div.innerText + "&direct=true" +"&page=" + page+'&entries-per-page='+maxResults+'&cached=true';
...
fileModal.addEventListener('show.bs.modal', function (event) {
    ...
    request = '/deps_for?path=' + deps_path + "&direct=true" + "&page=" + page + '&entries-per-page='+maxResults+'&cached=true';
    $.get(request, function (data) {
