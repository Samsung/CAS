...
function get_childs(path,id, node, page = 0, lm) {
    ...
    let deps_url = `/deps_of?path=${encodeURIComponent(path)}&page=${page}&direct=true`;
    ...
    for (let i = 0; i < depArray.length; i++) {
        ...
        if (depArray[i]["num_deps"] > 0) {
            div.append($('<span />').append($('<a href=/deps_tree?path=' + depArray[i]["path"] + ' target="_blank" title="Start from this process" style="text-decoration:none" />').text(" ↸ ")));
        }
        li.append(div);
        process.append(li);
    }
...
function searchModule(btn){
    ...
    window.location.replace("/deps_tree?filter="+search_str)
}