...
pub struct GlobalRefs<'a>(&'a [u64], &'a [globalref_data]);

impl<'a> GlobalRefs<'a> {
    pub fn new(refs: &'a [u64], data: &'a [globalref_data]) -> Self {
        Self(refs, data)
    }

    pub fn iter(&self) -> impl Iterator<Item = GlobalRef> {
        std::iter::zip(self.0.iter(), self.1.iter())
            .map(|(id, info)| GlobalRef::new(*id, info))
    }
}
...
impl<'a> GlobalRef<'a> {
    pub fn new(id: u64, data: &'a globalref_data) -> Self {
    ...
    ///
    pub fn info_iter(&self) -> impl Iterator<Item = GlobalRefInfo> {
